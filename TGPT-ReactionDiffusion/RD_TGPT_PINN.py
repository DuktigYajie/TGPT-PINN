...
class GPT(nn.Module):
    def __init__(self, layers, nu,rho, P, c_initial, f_hat, IC_u, resid_data, IC_data, BC_bottom, BC_top):
        super().__init__()
        self.layers = layers
    ...
    def lossR(self):
        ...
        u_xxtt = autograd.grad(u_x, x, torch.ones(x.shape[0],1).to(device),create_graph=True)[0]
        u_xx = u_xxtt[:,[0]]
